...
- **Une seule surface n'est plus mesurée** : l'écran des cartes d'une catégorie. Onze fichiers
  de test importent `test/support/geometrie.dart` — tableau des scores, bilan de tour, ossature
  de configuration, réglages, la face de carte et le nom d'une catégorie depuis le 26 août, le
  départage, l'annonce de tour et les deux écrans de fin le 27, puis l'étape des équipes et
  l'accueil. Là où rien ne mesure, un test de mise en page ne rougit que sur une exception de
  `RenderFlex` ou sur un widget introuvable, jamais sur un texte rogné.

  Ce que ces trous coûtaient n'avait rien de théorique : les ouvrir a trouvé **treize défauts
  en production** en deux jours — six sur les écrans de jeu et de fin, sept sur la configuration
  et l'accueil, dont le compteur d'équipes rogné dès ×1,6, un réglage Android ordinaire. Le
  tableau des mesures est dans `ROADMAP.md`.

  Et la liste des trous est celle des trous **connus**, pas des trous. Le départage n'y était
...
| #54 | L'étape des équipes et l'accueil sont mesurés à leur tour, et cachaient sept défauts de plus. Le compteur d'équipes était rogné **dès ×1,6** — un cran ordinaire d'Android — et c'est à dix équipes, le maximum que R8.1 promet, que le nombre passe à deux chiffres. L'accueil rognait ses entrées **par le bas**, sa cible tactile de 64 px ayant été prise pour un plafond, et son bloc d'identité débordait de 130 px à ×3,1. Mesurer une étape de configuration mesure aussi tout ce qui est empilé dessous : c'est ce qui a fait tomber le mode, les réglages et l'accueil sans les viser. |
